...
| Business scenario | Pattern | Why | Watch out for |
|---|---|---|---|
| Live price for a configured product | Request and Reply | The dealer is on the screen and cannot proceed without it | Set a timeout and a fallback price source, or your quote flow dies with the pricing engine |
...
| Invoice and payment history | Data Virtualization | Finance owns it, it is rarely queried, and copying it means owning the sync forever | Source availability, and query performance on wide date ranges |
...
| Warranty claim status | Remote Call-In, or Virtualization | Depends on whether the warranty system can push | If it cannot push, virtualization beats polling |
...
|---|---|---|
| Product configurator | Canvas | Must feel native, and needs the dealer, brand and territory context to configure correctly |
...
| Parts catalog | Depends on ownership. Native if you own it, Canvas if embedded, SSO if the vendor owns the experience | The deciding question is who owns the interface |
